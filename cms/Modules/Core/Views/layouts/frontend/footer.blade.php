<footer>
    <div class="bg_footer">
        <img class="logo_back_top" alt="Background footer" src="{{asset('frontend/images/config/bg_strengths_1698730971.jpg')}}"
            srcset="https://mamashomestay.com/images/config/bg_strengths_1698730971.jpg.webp">
    </div>
    <div class="top-ft cls">
        <div class="container1170">
            <a href="index.html" title="HaGiangMountain’s Homestay - Explore Ha Giang" class="logo_footer" rel="home">
                <img class="logo_back_top" width="263px" height="242px" src="{{asset('frontend/images/config/asset-18_1698834504.svg')}}"
                    alt="HaGiangMountain’s Homestay - Explore Ha Giang">
            </a>
            <div class="footer_info">
                <p>Discover Tranquil Moments, Embrace Genuine Hospitality – Welcome to HaGiangMountain's Homestay</p>
                <div class="footer_address">
                    <div class="address">
                        <div class="title">
                            Contact </div>
                        <p class="text_add">Address: No.124, 20/8 street, Ha Giang, Vietnam</p>
                        <p class="text_map">- See above <a href="https://maps.app.goo.gl/BKP54Si47wXMq5Wa9"
                                target="blank" title="address">Google Map</a></p>
                        <p class="hotline_add">Hotline Whatsapp 24/7: <a target="_blank"
                                href="https://wa.me/+84849278787">+84849278787</a></p>
                        <p>Email: <a href="mailto:HaGiangMountainshomestayhg@gmail.com">HaGiangMountainshomestayhg@gmail.com</a></p>
                    </div>
                    <div class="social">
                        <div class="title">
                            instagram
                        </div>
                        <div class="qr-img">
                            <img class="logo_back_top" alt="HaGiangMountain’s Homestay - Explore Ha Giang" width="151px"
                                height="135px" src="images/config/qr_1699688675.png"
                                srcset="https://mamashomestay.com/images/config/qr_1699688675.png.webp">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="coppy-right">
        <div class="">
            <div class="container coppy-right_block">
                <div class="cr-left">
                    <p>©2023 Allrights reserved HaGiangMountain’s Tour</p>
                </div>
            </div>
        </div>
    </div>
</footer>
